...
```{r echo = FALSE, message=FALSE, warning=FALSE, error=FALSE,fig.width=7, fig.height=4}
ggplot(train, aes(x=reorder(MSZoning, SalePrice, FUN = median), y=SalePrice))+
  geom_boxplot()+
  theme_classic()+
  scale_y_continuous('House Prices',labels = dollar)
```
...
```{r echo = FALSE, message=FALSE, warning=FALSE, error=FALSE,fig.width=7, fig.height=4}
ggplot(train, aes(x=reorder(Foundation, SalePrice, FUN = median), y=SalePrice))+
  geom_boxplot()+
  theme_classic()+
  scale_y_continuous('House Prices',labels = dollar)
```
...
#### 3.2.1 Getting the age of the property as of 2018
```{r}
...
#### 3.2.2 Assigning a varibale reovate to every property
```{r}
...
#### 3.2.3 Putting houses in different sizes
```{r}
...
#### 3.2.5 Number of Years from when the property was Sold
```{r}
...
#### 4.1 Creating Dummy Variables for Regression Model
```{r}
...
#### 4.2 Zero- and Near Zero-Variance Predictors
```{r}
...
#### 4.4 Dropping a variable month sold
```{r}
...
#### 3.4.4 Centering and Scaling variables and PCA
```{r}
