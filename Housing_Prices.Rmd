...
ggplot(full, aes(x=PropAge, y=SalePrice, colour=factor(Renovate)))+
  geom_point()+
  scale_y_continuous('House Prices',labels = dollar)+
  theme_minimal()
```
...
#### 6.3 Feature 3: Price per sqft of living area
```{r echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.width=7, fig.height=4}
full$PriceSQFT <- round(full$SalePrice / full$GrLivArea, 1)

full_notnull <- filter(full, !is.na(SalePrice))

ggplot(full_notnull, aes(x=reorder(Neighborhood, PriceSQFT, FUN = median), y=PriceSQFT))+
  geom_boxplot()+
  theme_classic()
```
