...
#### 5.3 Year Built, Year Renovated, Year Garage Built and Year Sold
```{r}
full$YearBuilt <- factor(full$YearBuilt)
full$YearRemodAdd <- factor(full$YearRemodAdd)
full$GarageYrBlt <- factor(full$GarageYrBlt)
full$YrSold <- factor(full$YrSold)
```

#### 5.4 Total number of rooms in living area
```{r}
full$TotRmsAbvGrd <- factor(full$TotRmsAbvGrd)
table(full$TotRmsAbvGrd)
```

#### 5.5 Total number of bathrooms in living area
```{r}
...
#### 5.6 Extracting factor variables
```{r}
...
full_notnull <- filter(full, !is.na(SalePrice))

ggplot(full_notnull, aes(x=reorder(Neighborhood, SalePrice, FUN = median), y=SalePrice))+
  geom_boxplot()+
  theme_classic()+
  scale_y_continuous('House Prices',labels = dollar)
```
...
#### 5.8 Number of rooms in respect to house prices
```{r}
...
#### 5.9 Number of bathrooms in respect to house prices
```{r}
...
#### 5.10 Number of houses built per year
```{r echo = FALSE, message=FALSE, warning=FALSE, error=FALSE, fig.width=7, fig.height=4}
ggplot(full, aes(YearBuilt))+
  geom_bar()+
  theme_classic()+
...
#### 6.2 Feature 2: Assigning a variable Reovate to every property
```{r}
...
#### 7.4 Feature 4: Number of Years from when the property was Sold
```{r}
...
#### 7.3 Zero- and Near Zero-Variance Predictors
```{r}
